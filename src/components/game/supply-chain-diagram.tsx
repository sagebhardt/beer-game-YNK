...
export function SupplyChainDiagram({ playerRole, className }: SupplyChainDiagramProps) {
          ...
          return (
            <div key={nodeId} className="flex items-center gap-1">
              ...
              >
                ...
                <Icon className={cn("h-4 w-4", isPlayer ? "text-[var(--accent)]" : "text-[var(--text-muted)]")} />
                <span className={cn("mt-0.5 text-[10px] font-semibold leading-tight", isPlayer ? "text-[var(--accent)]" : "text-[var(--text-body)]")}>
                  {label}
                </span>
                {contextLabel && (
